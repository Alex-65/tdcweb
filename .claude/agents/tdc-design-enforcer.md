...
- Creates spec documents in `docs/superpowers/specs/`
...
## BRAINSTORMING PHASE (for non-trivial features)
...
- Review: `tdc-code-reviewer`, `tdc-design-system-enforcer` (for UI)
...
5. **Save the spec** under `docs/superpowers/specs/`.
...
### What Development Experts Check

Before writing code for a feature that has a design spec, development agents:
...
If you see these patterns on a feature that needs a design, raise the design question:
...
A 10-minute design session saves hours of rework. Match the depth of the design phase to the blast radius of the change.
